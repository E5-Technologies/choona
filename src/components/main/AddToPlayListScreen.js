        ...
        if (res.status === 200) {
          if (
            ...
            if (props.registerType === 'spotify') {
              ...
              isInternetConnected()
                .then(() => {
                  props.addSongsToPlayListRequest(
                    props.registerType,
                    addToPlayListObj,
                  );
                })
      ...
      NativeModules.Print.printValue(newtoken.pop())
        .then(res => {
          ...
          if (res === '') {
            ...
            toast(
              'Error',
              'This feature is available for users with Apple Music Subcription. You need to subscribe to Apple Music to avail this feature.',
            );
            setBool(false);
          } else {
            setDevToken(token);
        ...
        onPress={() => {
          if (props.registerType === 'spotify') {
            if (registerType === 'spotify') {
              let addToPlayListObj = {
                songUri: [
                  `spotify:track:${originalUri.substring(
