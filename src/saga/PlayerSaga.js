  ...
  try {
    if (items.registerType === 'spotify') {
      const response = yield call(
        getSpotifyApi,
        `${constants.spotifyPlayerBaseUrl}currently-playing`,
        spotifyHeader,
      );

      yield put({
        type: GET_CURRENT_PLAYER_POSITION_SUCCESS,
        data: response.data,
      });
    } else {
      const response = yield call(
...
export function* resumePlayerAction(action) {
  ...
  try {
    if (items.registerType === 'spotify') {
      const response = yield call(
        putSpotifyApi,
        `${constants.spotifyPlayerBaseUrl}play`,
        {},
        spotifyHeader,
      );
    ...
    yield put({
      type: RESUME_PLAYER_FAILURE,
      error: error.response.data.error.status,
    });
  }
}
...
export function* pausePlayerAction(action) {
  ...
  try {
    if (items.registerType === 'spotify') {
      const response = yield call(
        putSpotifyApi,
        `${constants.spotifyPlayerBaseUrl}pause`,
        {},
        spotifyHeader,
      );
...
export function* getSongFromIsrcAction(action) {
  try {
    ...
    let spotifyHeader = {
      Authorization:
        action.regType === 'spotify' ? `${spotifyToken}` : `${AppleToken}`,
    };

    if (action.regType === 'spotify') {
      const response = yield call(
        getSpotifyApi,
        `https://api.spotify.com/v1/search?q=isrc:${action.isrc}&type=track`,
        spotifyHeader,
      );
      yield put({
        type: GET_SONG_FROM_ISRC_SUCCESS,
...
export function* playListOfUserAction(action) {
  try {
    ...
    let spotifyHeader = {
      Authorization:
        'spotify' === 'spotify' ? `${spotifyToken}` : `${AppleToken}`,
    };
